...
def main():
    ...
    dir = os.getcwd() + '\\aero_shape_opt\\misc\\airfoil_gen'
    run_dir = os.getcwd() + '\\aero_shape_opt\\datasets\\xfoil_data_spline'
    n = 0
    for filename in os.listdir(dir):
        # Avoid the CP files
        if filename.endswith(".dat") and 'CP' not in filename:
            # Check if file has already run
            if os.path.exists(run_dir + '\\' + filename.split('.')[0] + '_airfoil.txt'):
                continue
...
def runXfoil(name,filename,numNodes,alphas,Re,M):
    ...
    if os.path.exists('aero_shape_opt\\datasets\\xfoil_data_spline\\' + saveFlnmAF):
        os.remove('aero_shape_opt\\datasets\\xfoil_data_spline\\' + saveFlnmAF)
    if os.path.exists(os.getcwd() + '\\aero_shape_opt\\datasets\\xfoil_data_spline\\' + saveFlnmPolar):
        os.remove('aero_shape_opt\\datasets\\xfoil_data_spline\\' + saveFlnmPolar)

    # Create the airfoil
    fid = open(xfoilFlnm,"w")
    fid.write("PLOP\ng\n\n")
    fid.write("load aero_shape_opt/misc/airfoil_gen/" + filename + "\n")
    fid.write("PPAR\n")
    fid.write("N " + numNodes + "\n")
    fid.write("\n\n")
    fid.write("PSAV aero_shape_opt/datasets/xfoil_data_spline/" + saveFlnmAF + "\n")
    fid.write("OPER\n")
    fid.write("VISC " + Re + " \n")
    fid.write("MACH " + M + " \n")
    fid.write("ITER 200\n")
    fid.write("PACC\n")
    fid.write("aero_shape_opt/datasets/xfoil_data_spline/" + saveFlnmPolar + "\n")
    fid.write("\n")
